...
public class ConnectionWrapper extends WrapperBase implements JdbcConnection {
    ...
    protected void close(boolean fireClosedEvent) throws SQLException {
        Lock pooledConnectionLock = this.pooledConnection.getLock();
        ...
        try {
            ...
            if (!isInGlobalTx() && this.mc.getPropertySet().getBooleanProperty(PropertyKey.rollbackOnPooledClose).getValue() && !getAutoCommit()) {
                rollback();
            }
    ...
    public Lock getConnectionLock() {
        return this.mc.getConnectionLock();
    }

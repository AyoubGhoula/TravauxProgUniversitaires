	...
	/*
	  ...
	  */  
	    ...
	    public String creat_tables(Connection conn) throws SQLException {
	        ...
	        String sql2 = """
	            CREATE TABLE IF NOT EXISTS SUPPLIMENTAIRE (
	                NOM_SUPPLIMENTAIRE VARCHAR(100) PRIMARY KEY,
	                PRIX DOUBLE
	            );
	        """;
	        
	        String sql3 = """
	            CREATE TABLE IF NOT EXISTS PETIT_DEJEUNER (
	                ID_PETIT_DEJEUNER INT PRIMARY KEY AUTO_INCREMENT,
	        ...
	        String sql4 = """
	            CREATE TABLE IF NOT EXISTS PETIT_DEJEUNER_SUPPLEMENT (
	                ID_PETIT_DEJEUNER INT,
	                NOM_SUPPLEMENT VARCHAR(100),
	                PRIX_SUPPLEMENT DOUBLE,
	                FOREIGN KEY (ID_PETIT_DEJEUNER) REFERENCES PETIT_DEJEUNER(ID_PETIT_DEJEUNER) ON DELETE CASCADE
	            );
	        """;
	    ...
	    public Product select_cafe_by_name(Connection conn, String nomCafe) throws SQLException {
	        ...
	        try {
	            ...
	            System.err.println("Erreur lors de la récupération du café: " + nomCafe);
	            throw e;
	        } finally {
	            if (rs != null) rs.close();
	            if (pstmt != null) pstmt.close();
	    ...
	    /**
	     * Select a viennoiserie by name without user input (for Builder pattern)
	     */
	    public Product select_viennoiserie_by_name(Connection conn, String nomViennoiserie) throws SQLException {
	        PreparedStatement pstmt = null;
	        ResultSet rs = null;
	        Product viennoiserie = null;
	        ...
	        try {
	            ...
	            System.err.println("Erreur lors de la récupération de la viennoiserie: " + nomViennoiserie);
	            throw e;
	        } finally {
	            if (rs != null) rs.close();
	            if (pstmt != null) pstmt.close();
	    ...
	    // INSERT Supplimentaire
	    public boolean insert_supplimentaire(Connection conn, PreparedStatement pstmt) throws SQLException {
	        try {
	            Scanner sc = new Scanner(System.in);
	            ...
	            String sql = "INSERT INTO SUPPLIMENTAIRE (NOM_SUPPLIMENTAIRE, PRIX) VALUES (?, ?)";
	            pstmt = conn.prepareStatement(sql);
	            ...
	            if (rowsAffected > 0) {
	                System.out.println("✓ Supplément '" + name + "' inséré avec succès");
	                return true;
	            }   
	            return false;
	        } catch (Exception e) {
	    ...
	    // SELECT ONE Supplimentaire
	    public Product select_supplimentaire(Connection conn, PreparedStatement pstmt) throws SQLException {
	        ResultSet rs = null;
	        Product supplimentaire = null;
	        
	        try {
	            Scanner sc = new Scanner(System.in);
	            
	            System.out.println("\n🔍 RECHERCHER UN SUPPLÉMENT");
	            System.out.print("Enter NOM_SUPPLIMENTAIRE: ");
	            String nomSupp = sc.nextLine();
	            
	            String sql = "SELECT * FROM SUPPLIMENTAIRE WHERE NOM_SUPPLIMENTAIRE = ?";
	            pstmt = conn.prepareStatement(sql);
	            pstmt.setString(1, nomSupp);
	            
	            rs = pstmt.executeQuery();
	            
	            if (rs.next()) {
	                supplimentaire = protectFactory.createProduct(
	                    rs.getString("NOM_SUPPLIMENTAIRE"),
	                    0,  // FO_ID not used
	                    rs.getDouble("PRIX"),
	                    0,  // VENTES not used
	                    "supplimentaire"
	                );
	                System.out.println("✓ Supplément '" + nomSupp + "' trouvé");
	                System.out.println("  - Prix: " + supplimentaire.getPRIX() + " DT");
	            } else {
	                System.out.println("✗ Supplément '" + nomSupp + "' non trouvé");
	            }
	            
	        } catch (SQLException e) {
	            System.err.println("✗ Erreur lors de la récupération du supplément");
	            e.printStackTrace();
	            throw e;
	        } finally {
	            if (rs != null) rs.close();
	    ...
	    // SELECT by name (for Builder pattern)
	    public Product select_supplimentaire_by_name(Connection conn, String nomSupp) throws SQLException {
	        PreparedStatement pstmt = null;
	        ResultSet rs = null;
	        Product supplimentaire = null;
	        ...
	        try {
	            ...
	            System.err.println("Erreur lors de la récupération du supplément: " + nomSupp);
	            throw e;
	        } finally {
	            if (rs != null) rs.close();
	            if (pstmt != null) pstmt.close();
	    ...
	    // UPDATE Supplimentaire
	    public boolean update_supplimentaire(Connection conn, PreparedStatement pstmt) throws SQLException {
	        try {
	            Scanner sc = new Scanner(System.in);
	            
	            System.out.println("\n✏️ MODIFIER UN SUPPLÉMENT");
	            System.out.print("Enter NOM_SUPPLIMENTAIRE à modifier: ");
	            String nomSupp = sc.nextLine();
	            
	            System.out.print("Nouveau prix: ");
	            double nouveauPrix = sc.nextDouble();
	            
	            String sql = "UPDATE SUPPLIMENTAIRE SET PRIX = ? WHERE NOM_SUPPLIMENTAIRE = ?";
	            pstmt = conn.prepareStatement(sql);
	            
	            pstmt.setDouble(1, nouveauPrix);
	            pstmt.setString(2, nomSupp);
	            
	            int rowsAffected = pstmt.executeUpdate();
	            
	            if (rowsAffected > 0) {
	                System.out.println("✓ Supplément '" + nomSupp + "' mis à jour avec succès");
	                return true;
	            } else {
	                System.out.println("✗ Supplément '" + nomSupp + "' non trouvé");
	                return false;
	            ...
	            System.err.println("✗ Erreur lors de la mise à jour du supplément");
	            e.printStackTrace();
	            throw e;
	        }
	    }

	    // DELETE Supplimentaire
	    public boolean delete_supplimentaire(Connection conn, PreparedStatement pstmt) throws SQLException {
	        try {
	            Scanner sc = new Scanner(System.in);
	            ...
	            String sql = "DELETE FROM SUPPLIMENTAIRE WHERE NOM_SUPPLIMENTAIRE = ?";
	            pstmt = conn.prepareStatement(sql);
	            pstmt.setString(1, nomSupp);
	            ...
	            if (rowsAffected > 0) {
	                System.out.println("✓ Supplément '" + nomSupp + "' supprimé avec succès");
	                return true;
	            } else {
	                System.out.println("✗ Supplément '" + nomSupp + "' non trouvé");
	                return false;
	            ...
	            System.err.println("✗ Erreur lors de la suppression du supplément");
	            e.printStackTrace();
	            throw e;
	        }
	    }

	    // SELECT ALL Supplimentaires
	    public List<Product> selectAll_supplimentaire(Connection conn) throws SQLException {
	        try {
	            System.out.println("\n📋 LISTE DE TOUS LES SUPPLÉMENTS");
	            System.out.println("=".repeat(80));
	            
	            List<Product> supplimentaires = selectAll(conn, "SUPPLIMENTAIRE", "supplimentaire");
	            
	            if (supplimentaires.isEmpty() || supplimentaires == null) {
	                System.out.println("⚠ Aucun supplément trouvé dans la base de données");
	            } else {
	                System.out.println("✓ " + supplimentaires.size() + " supplément(s) trouvé(s):\n");
	                
	                System.out.printf("%-40s %-15s%n", "NOM", "PRIX (DT)");
	                System.out.println("-".repeat(80));
	                
	                for (Product supp : supplimentaires) {
	                    if (supp == null) {
	                        break;
	                    }
	                    System.out.printf("%-40s %-15.2f%n", supp.getName(), supp.getPRIX());
	                }
	                System.out.println("=".repeat(80));
	            }
	            
	            return supplimentaires;
	            
	        } catch (SQLException e) {
	            System.err.println("✗ Erreur lors de la récupération des suppléments");
	            e.printStackTrace();
	            throw e;
	        }
	    }

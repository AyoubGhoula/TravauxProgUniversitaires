...
public class PetitDejeunerConcreteBuilder implements PetitDejeunerBuilder {
    ...
    public void addSupplimentaireFromDB(String nomSupp) {
        try {
            SingConnection sing = SingConnection.getSingConnection();
            Connection conn = sing.getConnection();
            
            Product suppProd = gestion.select_supplimentaire_by_name(conn, nomSupp);
            
            if (suppProd != null) {
                petitDejeuner.addSupplimentaire((Supplimentaire) suppProd);
                System.out.println("✓ Supplément '" + nomSupp + "' ajouté au petit déjeuner");
            } else {
                System.out.println("✗ Supplément '" + nomSupp + "' non trouvé dans la base de données");
            }
        } catch (SQLException e) {
